---
name: harness
description: "Codex-native 하네스를 구성, 확장, 점검합니다. 현재 저장소에 맞는 `AGENTS.md`, `.agents/skills/*/SKILL.md`, `.codex/config.toml`, `.codex/agents/*.toml`를 생성하거나 재구성할 때 사용합니다. '하네스 구성해줘', '에이전트 팀 설계해줘', '하네스 점검', '에이전트/스킬 동기화', '코드 리뷰 하네스 만들어줘' 같은 요청에서 적극적으로 사용합니다."
---
...
실행 모드는 다음 중 하나로 표기한다:

- **Codex custom subagents**: `.codex/agents/*.toml` 역할을 오케스트레이터가 필요할 때 호출하도록 설계한다. 기본값이다.
- **오케스트레이터 단독**: 작업이 작고 분업 가치가 없을 때 사용한다.
- **하이브리드**: 수집/분석/검증 Phase별로 역할 활성화 방식을 다르게 한다.

2개 이상의 독립 관점이 품질을 높이면 다역할 하네스를 쓰고, 결과물 하나를 선형으로 만드는 작은 요청이면 오케스트레이터 단독 또는 producer-reviewer로 축소한다.
...
각 역할은 standalone custom subagent 파일인 `.codex/agents/{name}.toml`로 남긴다.
...
에이전트 목록과 스킬 전문은 `.codex/agents/`와 `.agents/skills/`가 단일 출처다. `AGENTS.md`에 중복해서 길게 쓰지 않는다.
...
- `.codex/config.toml`의 `web_search`, `[features]`, `[agents]` 설정이 올바른지
...
| 피드백 | 수정 대상 |
|--------|----------|
| 역할이 겹침 | `.codex/agents/*.toml`, 오케스트레이터 활성화 기준 |
...
```json
{
  ...
  "execution_mode": "codex-custom-subagents",
  "workspace_root": "_workspace",
  "web_search": "disabled",
  "orchestrator": {
    "name": "code-reviewer",
    "description": "넓은 범위의 코드 리뷰 하네스나 역할 기반 리뷰 팀이 필요할 때 사용한다. 후속 리뷰, 부분 재검토, 기존 리뷰 보완 요청에도 사용한다."
  },
  "agents": [
    {
