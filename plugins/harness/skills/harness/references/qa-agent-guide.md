...
### 3-1. 읽기 전용 역할이 아닌 검증 실행 가능한 역할로 정의하라

QA 에이전트가 읽기 전용 분석 역할이면 코드와 문서를 볼 수는 있지만 효과적인 QA에 필요한 실행 검증이 제한된다. 효과적인 QA는:
- Grep으로 패턴 검색 (모든 `NextResponse.json()` 추출)
- 스크립트 실행으로 자동 대조 (API shape vs 훅 타입)
- 필요 시 수정까지 가능

**권장**: `.codex/agents/qa-engineer.toml`에 검증 실행 권한과 "검증 → 리포트 → 수정 요청" 프로토콜을 명시한다. 실제 코드 수정까지 맡길지, 리포트만 맡길지는 하네스의 위험도에 맞춰 정한다.
...
- 발견 즉시 부모 스레드 또는 `_workspace/` 검증 리포트에 구체적 수정 요청을 기록한다. 형식은 파일:라인 + 수정 방법이다.
- 경계면 이슈는 생산자/소비자 양쪽 역할이 모두 읽을 수 있도록 공유 워크스페이스에 남긴다.
- 오케스트레이터에게 통과/실패/미검증 항목을 구분해 반환한다.
```
