...
- Required: Codex version, install mode, reproduction steps, expected vs actual, OS.
...
- Open an issue using the **Question** form, **or** start a thread in [GitHub Discussions](https://github.com/revfactory/codex-harness/discussions) if the matter is open-ended.
...
- Node.js `>= 18` if you run optional markdown or YAML tooling locally
- Git
...
To test your changes in a local Codex session without publishing to the marketplace:

```bash
# From your checkout
codex plugin link ./plugins/harness

# Verify
codex plugin list | grep harness
```

Unlink with `codex plugin unlink harness` when you're done.

### Running the meta-skill

```bash
codex "use this codex-harness repo as a reference and build a harness for a fintech risk-assessment team"
```

Scaffolded agents and skills land under `.codex/agents/` and `.agents/skills/` in the target project, with `AGENTS.md` as the short pointer.

### Tests & lints

- Markdown lint: `npx markdownlint '**/*.md'`
- YAML lint (issue templates & workflows): `npx yaml-lint .github/`
- Scaffold smoke test: `python3 plugins/harness/skills/harness/scripts/scaffold_harness.py --spec examples/code-reviewer/spec.json --target /tmp/codex-harness-demo`
